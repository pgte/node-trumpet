        ...
        if (name === 'open') {
            ...
            selectors.forEach(function (sel) {
                ...
                sel.pending = sel.pending.filter(function (p) {
                    var run = level < p.level;
                    if (run) {
                        if (sel.updating) pendingUpdates ++;
                        p.callback.call(stream, p.buffered, done);
                    }
                    return !run;
                });
...
function createSelector (selector, fn, special) {
    ...
    var sel = function (tag, parser) {
        ...
        function updateOrReplaceFunction (removing) {
            return function (cb) {
                if (expired) expire();
                
                p.buffered = '';
                p.callback = function (html, done) {
                    var self = this;

                    function finished(value) {
                        console.log('finished');
                        if (value instanceof Stream) {
                            if (! done) { throw new Error('I need a callback here'); }
                            value.on('data', function(d) { self.emit('data', d) })
        ...
        fn({
            ...
            update : updateOrReplaceFunction(false),
            replace : updateOrReplaceFunction(true),
            remove : function () {
                if (expired) expire();
                sel.updating = true;
                sel.removing = true;
                p.callback = function (_, done) {
                    sel.updating = false;
                    done();
                };
                sel.pending.push(p);
